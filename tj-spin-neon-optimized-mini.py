...
Run: python3 tj-spin-neon-optimized.py [--fps 16] [--no-color]
...
No dependencies. Uses cached frames and changed-row spans with grouped colors.
Small panes automatically use a tighter fit with extra letter separation.
"""
import argparse
import os
import subprocess
...
def neon_color(x, y, phase, face):
    stops = ((80, 230, 255), (163, 112, 255), (255, 112, 207))
    t = max(0, min(1, y * .85 + x * .15 + .035 * math.sin(phase))) * 2
    i = min(1, int(t))
    mix = t - i
    base = [stops[i][j] * (1 - mix) + stops[i + 1][j] * mix for j in range(3)]
    u = (phase / math.pi) % 1
    sweep = -.2 + 1.4 * u
    distance = (x * .65 + y * .35) - sweep
    glint = .88 * math.exp(-.5 * (distance / .04) ** 2)
    shade = 1 if face else .58
    return tuple(round((v * (1 - glint) + 255 * glint) * shade) for v in base)
...
def render(objects, phase, cols, rows, color):
    width, height = min(cols - 1, 67), min(rows - 1, 35)
    if width < 16 or height < 10:
        return ()
    small = cols <= 45 or rows <= 22
    if small:
        # Fit the actual front-to-edge swing, not a hypothetical full turn.
        # Extra hook clearance prevents the two silhouettes merging at low resolution.
        lift = 1.8
        xmin, xmax = -5.50, 2.02
        ymin, ymax = (-4.5 - lift) * 1.3, 4.95 * 1.3
        sy = (height - 2.2) / (ymax - ymin)
        sx = .85 * min((width - 3) / (xmax - xmin), 3.2 * sy)
        center_x, center_y = (xmin + xmax) / 2, (ymin + ymax) / 2
    else:
        lift = 0
        sy = min((width - 2) / 31, (height - 2) / 16)
        sx = 2 * sy
        center_x = center_y = 0
    grid = [[' '] * width for _ in range(height)]
    depth = [[-float('inf')] * width for _ in range(height)]
    for points, cx, cy, direction in objects:
        cy = cy - lift if direction == 1 else cy
        a = direction * motion_angle(phase)
        c, s = math.cos(a), math.sin(a)
        for x, y, z, ch in points:
            rx, rz = x * c + z * s, -x * s + z * c
            col = round((width - 1) / 2 + (rx + cx - center_x) * sx) if small else round(width / 2 + (rx + cx) * sx)
            row = round((height - 1) / 2 + ((y + cy) * 1.3 - center_y) * sy) if small else round(height / 2 + (y + cy) * 1.3 * sy)
            if 0 <= col < width and 0 <= row < height and rz > depth[row][col]:
                depth[row][col] = rz
                grid[row][col] = ch
    result = []
    for row, cells in enumerate(grid):
        colors = bytearray(width)
        if color:
            for col, ch in enumerate(cells):
                if ch != ' ':
                    rgb = neon_color(col / max(width - 1, 1), row / max(height - 1, 1), phase, ch == '@')
                    r, g, b = (round(v / 51) for v in rgb)
                    colors[col] = 16 + 36 * r + 6 * g + b
        result.append((''.join(cells).encode('ascii'), bytes(colors)))
    return tuple(result)


def encode_delta(previous, current, cols, rows):
    """Paint one changed span per row; omit unchanged rows and repeated colors."""
    if not current:
        return b'' if previous == current else b'\x1b[HResize pane to 17x11+'
    width, height = len(current[0][0]), len(current)
    left, top = max(0, (cols - width) // 2), max(0, (rows - height) // 2)
    output = bytearray()
    active_color = None
    for row, (chars, colors) in enumerate(current):
        old_chars, old_colors = previous[row] if previous else (b' ' * width, bytes(width))
        if chars == old_chars and colors == old_colors:
            continue
        start = 0
        while chars[start] == old_chars[start] and colors[start] == old_colors[start]:
            start += 1
        end = width - 1
        while chars[end] == old_chars[end] and colors[end] == old_colors[end]:
            end -= 1
        ...
        for col in range(start, end + 1):
            if chars[col] != 32:
                ...
                if code != active_color:
                    output.extend(f'\x1b[38;5;{code}m'.encode() if code else b'\x1b[39m')
                    active_color = code
            output.append(chars[col])
    if output:
    ...
    try:
        ...
        while True:
            ...
            if current_size != size:
                ...
                previous_frame = None
                output.write(b'\x1b[0m\x1b[2J')
            frame = int(elapsed / 5.76 * 96) % 96
            if frame != previous_frame:
                if frame not in frames:
                    frames[frame] = render(objects, frame * math.tau / 96, *size, not args.no_color)
                key = (previous_frame, frame)
                payload = transitions.get(key)
                if payload is None:
                    before = frames.get(previous_frame)
                    payload = encode_delta(before, frames[frame], *size)
